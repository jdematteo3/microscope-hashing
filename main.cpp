...
int main()
{
	...
	voltageFile.open("distCal.csv");
	voltageFile << "Voltage,Distance,Computed Distance, \n";
	...
	for (int i = 0; i < npoint; i++)
		h.insert(h.begin(), 1 /((double) npoint));

	while (difftime(time(NULL), time1) < 100000)
	{
		...
		if (count == 0) {
			zero = pos;
			for (int i = 0; i < npoint; i++)
				volts.insert(volts.begin(), value);
		}
		...
		dist = -(pos - zero);
		double calcDist = CoeffsHashing::getDist(measurement);
		if(count % 50 == 0)
			std::cout << measurement << ", " << calcDist << "\n";


		voltageFile << to_string(value) + "," + to_string(calcDist) + ",\n";
